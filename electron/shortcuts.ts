...
export class ShortcutsHelper {
  ...
  public registerGlobalShortcuts(): void {
    ...
    globalShortcut.register("CommandOrControl+1", () => {
      console.log("Command/Ctrl + 1 pressed. Setting screenshot mode to full screen.")
      this.deps.setScreenshotMode("full")
      const mainWindow = this.deps.getMainWindow()
      if (mainWindow) {
        mainWindow.webContents.send("screenshot-mode-changed", "full")
      }
    })

    globalShortcut.register("CommandOrControl+2", () => {
      console.log("Command/Ctrl + 2 pressed. Setting screenshot mode to left half.")
      this.deps.setScreenshotMode("left")
      const mainWindow = this.deps.getMainWindow()
      if (mainWindow) {
        mainWindow.webContents.send("screenshot-mode-changed", "left")
      }
    })

    globalShortcut.register("CommandOrControl+3", () => {
      console.log("Command/Ctrl + 3 pressed. Setting screenshot mode to right half.")
      this.deps.setScreenshotMode("right")
      const mainWindow = this.deps.getMainWindow()
      if (mainWindow) {
        mainWindow.webContents.send("screenshot-mode-changed", "right")
      }
    })
