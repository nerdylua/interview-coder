...
function moveWindowVertical(updateFn: (y: number) => number): void {
  ...
  const maxUpLimit = (-(state.windowSize?.height || 0) * 5) / 6
  const maxDownLimit =
    state.screenHeight + ((state.windowSize?.height || 0) * 5) / 6
...
function getScreenshotMode(): string {
  return state.screenshotHelper?.getScreenshotMode() || "full"
}

function setScreenshotMode(mode: "full" | "left" | "right"): void {
  state.screenshotHelper?.setScreenshotMode(mode)
}
